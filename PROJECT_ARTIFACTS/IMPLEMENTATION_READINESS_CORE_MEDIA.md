...
Scope: the 12 `specs/core/*` and `specs/media/*` module specs (8 core + 4 media) plus
...
| `MODULE_AUDIO.md` | 🟢 | 🟢 | Interface is fully specced (design locked); wiring to `MODULE_PIPELINE.md`'s `AudioLoop::open` / `AudioLoop::run` is explicit. Implementation is *intentionally* deferred behind the video milestone — this is a documented status, not a spec gap. |
...
- `MODULE_STREAM_PARAMS.md` "Congestion-Reactive Bitrate Control": the full
  two-tier (fast/slow path) policy and its config knobs. It reacts to
  congestion; it does not estimate bandwidth.
...
`Params` to the pipeline's `param_tx`," and `MODULE_SERVER.md`'s `Config`
held a `stream_mgr: Box<dyn stream::Manager>`. But `Pipeline.param_tx` (the
...
| No user-initiated bandwidth test in any spec | **Still true, now recorded as a deliberate decision** rather than an open question — see user story US-MC-8. There is no bandwidth estimator in v1 at all: the adaptive loop *reacts* to congestion (QUIC `smoothed_rtt` and `cwnd`, the server's own datagram-drop rate, client `stats` deltas) rather than measuring capacity, and an active probe would steal throughput from the stream it measures. Adding one later needs a new frame type and a new directive; it must not be folded into the R-CLI-13 HUD. |
...
3. **The ABI had no way to express optional traits.** The pipeline said "set
   ...
   `params_capability()` hookup at pipeline step 3g.
...
**No blockers found for core + media by this method.** Every `server::Config`
field has a named constructor at step 7, the `AbiErr` ↔ host-enum mapping is a
bijection in each of its four domains, all 40 TD rows have traced fixes, and
every cross-module contract examined names both a producer and a consumer. Two
limits on that statement, both structural rather than incidental: the
cross-module sweep asked one question — "for every consumer, who produces
this?" — which is blind to byte budgets, thread affinity, ownership arity,
stream direction, and the Layer-1/Layer-2 seam; and it was not exhaustive, so
"0 blockers" describes the method's yield rather than the specs.
