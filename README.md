...
| Phase | What lands |
|---|---|
| **v1** | Browser client over HTTP/3 + WebTransport (QUIC), with a **supported** WebSocket carrier on the same port — lower performance under loss (TCP head-of-line blocking), but the same stream tags, message framing, auth, liveness and close codes, so only the envelope differs. Modern browsers only (Chrome 107+, Edge 98+, Firefox 130+, Safari 16.4+; the floor is set by WebCodecs availability — Chrome 107 / Firefox 130 / Safari 16.4 — while the WebTransport carrier additionally needs Safari 26.4+). Connectivity is the user's network problem, and **which carrier you get follows from how you solve it**: LAN or an overlay/VPN that forwards UDP (WireGuard, Tailscale) carries QUIC end to end and keeps WebTransport; an HTTP-proxying tunnel such as Cloudflare Tunnel cannot (`cloudflared` proxies HTTP/1.1 and HTTP/2 to the origin), so every session behind one uses the WebSocket carrier permanently. |
