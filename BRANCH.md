...
- **`PROJECT_ARTIFACTS/GAP_TRIAGE.md`** — the competitive/feature gap review, triaged into eight open questions (`OQ-01`…`OQ-08`, each recording the decision and its consequences), a **closed findings** table that exists so settled decisions are not re-raised, and a recommended sequence. **All eight are now closed** (status column, re-checked 2026-09-11): the two no-root Linux capture add-ons and the `kms_egl` headless correction (OQ-08), the WebSocket carrier's promotion with carrier-generic liveness and a performance budget (OQ-01), the Linux mode-set decision plus client-driven output dims in device pixels (OQ-03), idle suspension (OQ-04), the egress guard and per-session pacing cap (OQ-05), and the T1/T1+ control surface (OQ-07). A second owner pass then went further than several of those recommendations: **audio is un-deferred on all three platforms and on by default** with the microphone included (OQ-02), **per-client quality tiers ship in v1** scoped to resolution and bitrate and solved without an ABI break (OQ-06), **FeatherDesk provisions a display when none exists** (OQ-08), and Cloudflare's terms were confirmed (OQ-01 item 5). No open question remains.
...
|---|---|---|
| Every relative markdown link resolves | walk all 196 `.md` files, resolve every non-`http` link target | 0 broken (622 relative links, re-run 2026-09-11 after the OQ-08 pass and the QA checklists landed) |
...
| Every config key has a consumer | for each `section.key` row in `MODULE_CONFIG.md`'s key-reference table, grep the rest of `specs/` for the bare key name | 82/82 |
