...
- **`PROJECT_ARTIFACTS/GAP_TRIAGE.md`** — the competitive/feature gap review, triaged into eight open questions (`OQ-01`…`OQ-08`, each recording the decision and its consequences), a **closed findings** table that exists so settled decisions are not re-raised, and a recommended sequence. **Seven of the eight are now resolved in the spec tree** (status column, re-checked 2026-09-11): the two no-root Linux capture add-ons and the `kms_egl` headless correction (OQ-08), the WebSocket carrier's promotion with carrier-generic liveness and a performance budget (OQ-01), the Linux mode-set decision plus client-driven output dims in device pixels (OQ-03), idle suspension (OQ-04), the egress guard and per-session pacing cap (OQ-05), and the T1/T1+ control surface (OQ-07). OQ-02's trigger was corrected to Linux-only, since the previous "all three OSes" condition could never fire under step 4 of the Migration Strategy below. **OQ-06 is the one still open** past its first step, and it is gated on batching the ABI break.
...
|---|---|---|
| Every relative markdown link resolves | walk all 194 `.md` files, resolve every non-`http` link target | 0 broken (615 relative links, re-run 2026-09-11 after the OQ-08 pass and the QA checklists landed) |
